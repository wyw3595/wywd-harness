...
#
# 2026-09-19 实测调参：30 → 60。一次真实任务（排查目录选择器那条链）在 30 轮
# 上截断，而它光"摸清项目结构"就花掉十几轮——探索成本吃掉了大半预算。
#
# **提高上限本身不花钱**：它是保险丝不是预算，模型做完就停；只有真跑才消耗。
# 所以宁可给宽——截断的代价是白烧掉前面所有轮，那个比多给几十轮贵得多。
MAX_AGENT_STEPS_DEFAULT = 60
...
- **先定位，再精读。** 找东西先用 fs_glob / fs_find 定位到具体文件和行，再 fs_read
  那一小段。对同一个文件反复跳读不同位置，通常是没想清楚要找什么——停下来先想。
- **别读"元信息"文件**（NEXT_SESSION.md / MEMORY.md / 事故记录 / CI 配置 / 打包配置），
  除非用户明确要求。它们讲的是"项目怎么演进的"，不是任务材料：很贵，而且不推进任务。
- **一次搜一个东西。** fs_find 的 pattern 是**单条**正则；要找一个以上的符号就分几次搜，
  或者先 fs_glob 看有哪些文件、再定点读。
