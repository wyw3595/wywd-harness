...
> ## ⛔ 先读这条：**不要自作主张 push**（2026-09-19 用户明确要求）
>
> **原话：「以后不要乱推送代码」**
>
> - **`git push` 绝不主动**——只有用户明确说「推送 / push / 提交到 github」才做；
> - **`git commit` 也默认不做**——做完工作就**报告 + 等指示**，别"顺手提交"；
> - 当次授权**只对那一次有效**，不要外推到后面的任务；
> - 报告时说清「改了哪些文件 / 测试什么状态 / 要不要我提交」。
>
> 为什么：push 是外部动作（上了 GitHub 别人就能拉），"做完就推"等于让用户在不清楚
> 改了什么的情况下被动接受。2026-09-19 之前几轮我连续这样做，他明确不认可。
> **这条优先于本文件里任何"做完就提交/推送"的旧描述。**
...
## 2026-09-17（续二）：s14 上下文压缩 —— 长任务的第二半之二

**为什么做它**：s13 让**单条**超大输出不再是上下文杀手；但只要对话够长，
`trim_history` 那 20 条硬截断迟早会把"用户最初的目标"和"已经确认的结论"
一起丢掉。s14 换成**分层压缩**：从最廉价的开始做，够用就停。

**新增 `src/harness/compact.py`**

| 层 | 策略 | 代价 |
|----|------|------|
| L1 | 超大工具结果截断（并**注明**被截断）| 低（不丢消息）|
| L2 | 同一文件重复读取只留最新 | 低（只换正文，**不删消息**）|
| L3 | 修剪旧消息，保 system + 首条 user + 最近 N 条 | 中（开始丢消息）|
| L4 | 生成式摘要替换中段 | 高（**唯一会额外花钱**的一层）|

外加 `DurableContextState`（`DurableFact` / `PendingItem`，frozen + tuple +
构造期校验）与 `render_durable_context()`——**无损旁路**，永不参与压缩。

**接线**：`sidecar.turn_runner` 与 `chat.py` 都从 `trim_history` 换成
`compact(history, budget_tokens=..., summarizer=model_summarizer(model),
keep=max_history)`。两道闸各管一件事：token 预算管"贵不贵"，条数上限管"太长"。
`trim_history` 保留在 memory.py（它是 L3 的基础实现，测试仍在用）。

**实现里踩到并修掉的三个坑（都值得记）**
1. **L2 绝不能删消息，只能换正文**。教材把旧的 `tool_result` block 整个删掉，
   但对应的 `tool_use` 还在——那是**孤儿调用**，OpenAI 兼容协议直接报错。
   我们只把正文换成"（这个文件后来又读过一次，内容已省略）"。
2. **L3/L4 用同一个 `keep` 会让 L4 永远跑不起来**：L3 把历史裁到"head + keep"
   之后，L4 的 body 长度正好等于 keep，"旧消息"恒为空 → 每轮直接返回。
   现在 L4 保留得比 L3 少一半，压的是**中段**，最近的内容仍保留原文。
3. **摘要是"纯空白串"时不算成功**：`not "   "` 是 False，光判真假值会漏掉。
   用一段空白换掉整段历史，比不压缩严重得多——那才是真的失忆。

**与教材的其他差异**：按本项目的 OpenAI 方言实现（L2 靠"从 assistant 的
`tool_calls` 反查参数"认文件，不给 block 打第二份标签）；L4 注入式
（`summarizer=None` 就不跑那层）；摘要挂进**已有的第一条 system**而不是
新插一条中间位置的 system（"system 只在开头"是更保守的约定）。

**验收**
- **541 条测试全绿**（511 + 30 条 `test_compact.py`）
- `.workbuddy/scratch/demo_compact.py` 三个场景，四层各出场一次：

| 场景 | 触发层 | 效果 |
|---|---|---|
| 大输出 + 重复读同一文件 | L1 + L2 | 8886 → 376 token（96%），**条数不变 18→18** |
| 又长又平淡（62 条） | L3 | 1363 → 85 |
| 每条都很长（14 条） | L3 + L4 | 4355 → 1123，摘要进第一条 system |

  三个场景都验了：无孤儿 tool 消息、用户最初的目标还在、**调用方的历史一字未改**。

**至此「长任务」这条线补齐了**：步数上限（30，可配 + 预算告知）+ 单条输出
...
## 2026-09-19（续）：系统提示词重写 —— 从 470 字到 1270 字

**为什么改**：原来的提示词只讲了两件事（两类工具 + 步数预算），而模型**猜不到**
的东西远不止这些——尤其是**边界**：它不知道写文件要审批、不知道越界会被拒、
更不知道 s13 之后它看到的 `[Artifact: ...]` 是什么意思。缺一条说明，模型就白撞
一次，代价是一次完整的工具往返。

**结构**（`scripts/toolbox.py` 的 `SYSTEM_PROMPT_TEMPLATE`，分节写在常量注释里）

| 节 | 写什么 | 出处 |
|---|---|---|
| 身份 | 一句话：能读代码、跑命令、改文件的执行者 | —— |
| 怎么做事 | 先查再答 / 改前先读 / 能并行就并行 / 报错先读报错 / 没做完就说没做完 | fs_edit 的真实约束、工具报错的文案设计 |
| 工具怎么用 | 两类工具 + 延迟工具目录 | s03 |
| 边界 | 工作目录 / 审批 / 硬性拒绝 / **Artifact 指针** | permissions 的规则、s13 的外化 |
| 预算 | 步数上限 + 接近上限时收尾 | 09-16"预算要讲出来"的结论 |
| 说话方式 | 结论先行、不编造 | —— |

**三条设计原则**（比内容本身更重要）

1. **判据是「删掉它，行为会不会变」**——不会变的就是废话（"要认真""要专业"）。
   提示词里每一句都在花每一轮的 token。
2. **不重复 schema 已有的信息**（工具参数在 schema 里）；**不硬编码用户偏好**
   （语言/风格归 s11 用户记忆管）——后者用测试钉死了。
3. **每一条都要有出处**：要么是真实踩过的坑，要么是代码里的硬边界。
   **提示词是契约，不是座右铭。**

**验收**：**571 条测试全绿**（568 + 3 条新契约：四类边界不得缺失 / 预算跟着参数
走 / 提示词里不得出现语言偏好）。

**过程记录（值得留）**：改 `build_system_prompt` 时 Edit 工具的**多行匹配连续失败**
（那段旧代码里有多个**字面** `\n`，转义层数对不上），改用脚本按行替换，
**结果脚本自己写错两次**：

1. 第一版：前向搜索**没有距离上限**，`head` 一路退到文件顶部 → **删掉了前 627 行**；
2. 第二版：替换范围写成了 `start`（应为 `head`）→ 漏掉 `return (` 那行 → 语法错误。

两次都是**从备份 / git 恢复**后重做的，**没有丢任何东西**。三条教训：
- **前向/后向搜索必须有距离上限**，找不到就停；
- **所有断言必须在落盘之前**（第一版是先写盘再断言，等于没防住）；
- **改之前先备份**——这条救了一次（`git checkout` 也能救，但会丢未提交的改动）。

## 2026-09-19（续三）：s16 技能系统 —— 先列目录，用到再展开
...
这和 s03 的**延迟工具**是同一个渐进披露思想的两个应用——那边是"工具 schema
不常驻，先 ToolSearch 再执行"，这边是"技能正文不常驻，先看目录再加载"。
区别：延迟工具由**模型**主动发现，技能由**触发词自动匹配**（服务端做，省一次往返）。

**权限铁律（本章最重要的一条）**

    有效权限 = Harness 基础权限 ∩ 已审核的 Skill manifest

manifest **只能收窄、不能放宽**——声明 `tools: [bash]` 不等于能跑 bash，
它仍要过 s04 的审批与 s23 的沙盒。`paths` 只约束能理解结构化 `path` 参数的工具；
bash 这类多用途工具**不能靠字符串扫描获得真正的隔离**。

**存储**：用户级 `~/.workbuddy/skills` + 项目级 `{workspace}/.workbuddy/skills`，
**同名时项目级覆盖用户级**（越具体的作用域越优先）。

**三处刻意的差异（都来自这个环境的现实）**

1. **不引入 PyYAML**。项目一直是零依赖纯标准库；为一份 frontmatter 装解析器
   不值得。所以写了个**受限解析器**（支持 `key: value` / 行内列表 / 块列表 /
   **任意深度**嵌套），遇到不支持的语法（多行字符串 `|`/`>`、锚点、流式映射）
   **明确报错**而不是猜。
2. **认宿主格式的字段别名**。WorkBuddy 的 SKILL.md 用 `name`/`description`，
   教材用 `title`/`summary`。不认别名的话，机器上已有的技能**一个都列不出来**
   （实测：项目级那 3 个技能最初全被拒）。别名在**字段白名单之前**归一化。
3. **正文有长度上限**（20K 字符）。不然"按需加载"一次就能吃掉几万 token，
   等于没做这个功能。

**接线**
- `build_history_seed(..., skill_index=...)`：技能**目录**独立成一条 system。
- `sidecar._inject_skills()`：每轮按触发词匹配，把**命中的正文**追加进本轮视图。
  两个取舍：**已注入过的跳过**（同一技能命中多次不重复塞）；**注入的正文会进
  transcript**（有意——重放时能看到"当时凭什么那么做"，可审计 > 那点存储）。
- `load_skill` 延迟工具：目录已告诉模型有哪些，用它取全文。**免审批**——
  读一份指南不需要设防，要设防的是指南里让你做的那些事。
- 索引**建一次、两处共用**（目录与正文必须是同一份视图，否则"列出来的"和
  "能加载的"可能对不上）。

**验收**
- **606 条测试全绿**（575 + 31 条 `test_skills.py`）
- `.workbuddy/scratch/demo_skills.py` 四步演示：两级索引（含项目级覆盖）、
  目录 vs 正文的字符账、触发词匹配、越界权限被拒
- 两个**真实的**示例技能已放进项目级目录：`git-commit`（本仓库提交规范）、
  `code-review`（评审清单，含本项目的硬约定）

**过程中修掉的两个真 bug（都是跑演示才发现的）**

1. **解析器只支持一层嵌套**，而教材的 `permissions.paths.read` 是**两层**——
   项目级技能因此被静默跳过（演示里显示成了用户级版本）。改成**按缩进递归**后
   支持任意深度。
2. **格式错误没带文件路径**，批量扫描时看不出是哪个技能坏了。改成在 `reload()`
   里统一加路径前缀。

**这个环境里已有的技能**：项目级目录下还有一个 `sidecar-capability`（宿主放的，
用 `name`/`description` 格式）——别名兼容之后它也能被正确列出来了。

## 2026-09-19（续四）：一次真实失败驱动的两处调整

**现场**（在 `chat.py` 里跑的真实任务）：**30 轮用尽、任务未完成**，
账单 `本问 tokens：937,375 输入 / 3,031 输出`（93.7 万输入）。

**钱花在哪**（从运行记录直接读出来的四条）

1. **读了元信息文件**：`NEXT_SESSION.md`（**135 KB**）、`MEMORY.md`、事故记录、
   `ci.yml`、`pyproject.toml`——这些讲的是"项目怎么演进的"，不是任务材料。
   光读它们就吃掉大量 token，而且不推进任务。
2. **探索没有顺序**：对 `native_dialog.py` 的读取是 380 → 300 → 260 → 140 → 440 → 560
   ——倒着跳，像在随便翻。
3. **`fs_find` 的 pattern 用错**：一次写了四个符号（`A|B|C|D`）→ 未找到；
   它接受的是**单条**正则，要搜多个东西得分开搜。
4. **30 轮里光"摸清项目结构"就花掉十几轮**——探索成本吃掉了大半预算。

**改了两处**

1. **轮数上限 30 → 60**（`MAX_AGENT_STEPS_DEFAULT`）。理由：**提高上限本身不花钱**
   ——它是保险丝不是预算，模型做完就停；而截断的代价是白烧掉前面所有轮，
   那比多给几十轮贵得多。
2. **系统提示词补三条**（都进「怎么做事」节，每条都对应上面一条教训）：
   - **先定位，再精读**——对同一文件反复跳读不同位置，通常是没想清楚要找什么；
   - **别读"元信息"文件**（除非用户明确要求）——很贵，而且不推进任务；
   - **一次搜一个东西**——`fs_find` 的 pattern 是单条正则。

**刻意没动的**：compact 阈值仍然是 60K。这次的 token 累计是**线性增长**
（30 轮 × 约 3.1 万/轮），**单轮都没到阈值**，所以压缩压根没触发——
降低阈值会更早丢细节，那是另一件事（要不要用"更早丢细节"换"更低成本"，
得看真实任务的分布，不能拍脑袋）。

## 2026-09-19（续五）：P0 修复 —— 压缩产物被当证据落盘，第二轮必炸

**现场**（用户报的）：会话 sess_0196 第二轮发不出去，报
`已落盘的消息前缀被改写——append-only 证据不可回改`，前端显示「本轮没能开始」。

**根因**：`session.py` 写明不变量「record.messages **单调增长**（run_turn 只追加；
**trim 在 turn_runner 里做、从不回写 Record**）」。而 s14 之后 `turn_runner` 返回的是
**`compact` 的产物**（压缩视图 + 本轮新增）→ **回写进了 Record**。压缩一旦真出手
（L1 截断 / L2 去重 / L3 修剪 / L4 摘要**任一**），回写前缀 ≠ 已落盘 → append-only
校验拒绝 → **整轮失败、会话卡死**。

**为什么藏了三天**：压缩只在超预算（60K）时才动，短会话压根不触发。

**修复**（`src/harness/sidecar.py` 的 turn_runner 返回处）

```python
appended = result.messages[len(view):]
return result.output, list(history) + appended   # 落盘原始历史 + 本轮新增
```

分工变成：**模型看压缩视图，证据记原始历史**。副产品：`_inject_skills` 展开的
技能正文也不再落盘——它每轮按需重生，本来就不该进证据。

**验证**
- `.workbuddy/scratch/repro_by_compact.py`（`budget=100` + `max_history=4` 逼 L3/L4
  出手）：**修复前第二轮炸 `TranscriptCorruptionError`；修复后通过**，且压缩日志
  照打（`782 → 9 token：L3 修剪 -390；L4 摘要 -383`）——说明确实触发了又被正确处理。
- `tests/test_sidecar.py` 新增 `CompactionTranscriptTests` 两条回归
  （压缩出手时落盘仍 append-only / 预算充裕时不受影响）。
- **608 条全绿**（606 + 2）。

**顺带修了一条脆弱测试**：`test_empty_user_memory_adds_no_message` 调
`build_history_seed()` 时没传 `root`，会读到**真实的**工作区记忆；而 sess_0196
那次 `memory_write` 恰好让它变成非空 → 测试红了。改成 root 也指向临时目录。
**教训：测试不该依赖会变的外部状态。**

**排查手法（值得复用）**：单点假设逐个用**实证**排除（store 两轮 save ✅、
compact 预算内不改 ✅、`_inject_skills` 只 append ✅、`run_agent` 不改前缀 ✅、
`_commit_transcript_if_running` 干净 ✅、用真实会话内容重放 ✅），
最后**压低阈值/保留条数把压缩逼出手**，一次复现。

**状态**：改动**未提交、未推送**——用户明确要求不要自作主张推，等他说。
